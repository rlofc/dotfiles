...
CAPSULES_PROMPT=${CAPSULES_PROMPT:="git-uptream-remote-logo ssh path scm python_venv ruby node terraform cloud duration exit"}

# Theme custom glyphs
# SCM
SCM_GIT_CHAR_GITLAB=${CAPSULES_GITLAB_CHAR:='  '}
SCM_GIT_CHAR_BITBUCKET=${CAPSULES_BITBUCKET_CHAR:='  '}
SCM_GIT_CHAR_GITHUB=${CAPSULES_GITHUB_CHAR:='  '}
SCM_GIT_CHAR_DEFAULT=${CAPSULES_GIT_DEFAULT_CHAR:='  '}
SCM_GIT_CHAR_ICON_BRANCH=${CAPSULES_GIT_BRANCH_ICON:=''}
SCM_HG_CHAR=${CAPSULES_HG_CHAR:='☿ '}
SCM_SVN_CHAR=${CAPSULES_SVN_CHAR:='⑆ '}
# Exit code
EXIT_CODE_ICON=${CAPSULES_EXIT_CODE_ICON:=' '}
# Programming and tools
PYTHON_VENV_CHAR=${CAPSULES_PYTHON_VENV_CHAR:=' '}
RUBY_CHAR=${CAPSULES_RUBY_CHAR:=' '}
NODE_CHAR=${CAPSULES_NODE_CHAR:=' '}
TERRAFORM_CHAR=${CAPSULES_TERRAFORM_CHAR:="❲t❳ "}
# Cloud
AWS_PROFILE_CHAR=${CAPSULES_AWS_PROFILE_CHAR:=" aws "}
SCALEWAY_PROFILE_CHAR=${CAPSULES_SCALEWAY_PROFILE_CHAR:=" scw "}
GCLOUD_CHAR=${CAPSULES_GCLOUD_CHAR:=" google "}

# Command duration
COMMAND_DURATION_MIN_SECONDS=${COMMAND_DURATION_MIN_SECONDS:-1}

# Ssh user and hostname display
SSH_INFO=${CAPSULES_SSH_INFO:=true}
HOST_INFO=${CAPSULES_HOST_INFO:=long}
...
function __prompt-command() {
	...
	# Generate prompt
	PS1="\n "
	for segment in $CAPSULES_PROMPT; do
		local info
